...
[^2]: This is mainly becuase partial derivatives of the Jacobian matrix are non-zero when it comes to non-linear parameter transformations. In differential geometry, affine connections are introduced to handle this issue. Given a metric, the metric compatible connection is known as Levi-Civita connection (or Christoffel symbols if we use a coordinate representation).
