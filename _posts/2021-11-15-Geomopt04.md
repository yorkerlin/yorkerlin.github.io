...
This update in Eq. `$\eqref{1}$` is inspired by the standard linear update in the **tangent vector space** (as the green space in the above figure).
...
enough (as the red space in the above figure).
