...
By the standard chain rule, we have the parameter transform rule of Euclidean gradients as 

`$$
\begin{aligned}
\mathbf{g}_\tau =  \mathbf{g}_\lambda \mathbf{J}
\end{aligned},
$$` where `$\mathbf{g}_\tau$` is a  Euclidean gradient under parameterization `$\tau$` and `$\mathbf{J}$` is the Jacobian matrix defined at Eq. `$\eqref{5}$`.

It can be shown that the length of a Euclidean gradient is not invariant under a parameter transform (i.e., `$\| \mathbf{g}_\tau \| \neq \| \mathbf{g}_\lambda \|$`). The main reason is that we should use the [weighted inner product]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#distance-induced-by-the-fisher-rao-metric) to define the length of a gradient vector.
