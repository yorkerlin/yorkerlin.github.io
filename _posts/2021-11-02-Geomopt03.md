...
From the above expression, we can see that directional derivatives should be the same. (**geometric property** of directional derivatives)
`$$
\begin{aligned}
...
By `$\eqref{3}$` and `$\eqref{4}$`, we have the following identity obtained from the **geometric property** of directional derivatives.
`$$
\begin{aligned}
...
Eq. `$\eqref{6}$` gives us the transformation rule for Eulcidean gradients as below in a vector form.
...
We can see that the length of a Riemannian vector is also invariant. (**geometric property** of the length of a Riemannian vector)
`$$
\begin{aligned}
...
Finally, we can show that the optimal solution of [the problem ]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-steepest-direction) consider in Part II  under parametrization $\tau$ and $\lambda$ are equivalent since both the length and the directional derivative remain the same.
`$$
\begin{aligned}
