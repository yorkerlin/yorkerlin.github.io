...
Consider a vector function `$\lambda(\tau)$` from  input space `$\tau$` to output space `$\lambda$`.
The linearlization of the function is a linear map defined by the Jacobian matrix `$J$`.
This linear map is known as the pullback map of `$\lambda(\tau)$`   in differential geometry.
...
This linear map is known as the pushforward map of `$\tau(\lambda)$`  in differential geometry.
