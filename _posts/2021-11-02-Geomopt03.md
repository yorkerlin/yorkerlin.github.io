...
[^2]: We assume readers are familar with the transformation rule for Euclidean gradients. In differential geometry, this transformation rule can also be shown that `$\tau(t)=\gamma_\tau(t)$` and `$\mathbf{J}= \Psi_\lambda \circ \Psi_\tau^{-1}$`, where $\Psi_\tau$ is known as the coordinate map for parametrization $\tau$.
