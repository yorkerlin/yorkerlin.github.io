...
Working in Progress (to be polished)

# Parameter Transform and Invariance
------
In [Part II]({{ site.baseurl }}{% post_url 2021-10-04-Geomopt02 %}#riemannian-gradients-as-tangent-vectors-optional), we show that a Riemannian gradient is a parametric representation of the tangent direction of a curve in the manifold.
...
The corresponding coordinate-dependent argument is geometric properties should remain unchanged if we perform a (valid) parameter transform.
Now, we discuss one key **geometric property**: the directional derivative should remain the same if we perform a  parameter transform.
