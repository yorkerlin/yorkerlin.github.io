...
By Eq `$\eqref{3}$`, we obtain the transformation rule for Riemannian gradients  (denoted by a column vector) as below,  where `$\mathbf{Q}:=\mathbf{J}^{-1}$` is also a Jacobian matrix and `$Q_{ki}=\frac{\partial \tau^k(\lambda)}{\partial \lambda^i}$` is the `$(k,i)$` entry of matrix `$\mathbf{Q}$`. As disscused in [Part I]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#intrinsic-parameterizations),
the Jacobian matrix is always non-singular for these two intrinsic parameterizations.
