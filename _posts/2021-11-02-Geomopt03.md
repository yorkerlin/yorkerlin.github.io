...
\mathbf{g}_\tau =  \mathbf{g}_\lambda \mathbf{J}
\end{aligned},
$$` where `$\mathbf{g}_\tau$` and `$\mathbf{g}_\lambda$` are the Euclidean gradient under parameterization `$\tau$` and `$\lambda$`, respectively,  and `$\mathbf{J}$` is the Jacobian matrix defined at Eq. `$\eqref{5}$`.
