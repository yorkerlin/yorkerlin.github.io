...
More about this work [lin2021tractable]: [Long talk]( https://download.dsf.tuhh.de/ig4ds22/videos/IG4DS-WuLin.mp4),  [(Youtube) short talk](https://www.youtube.com/watch?v=vEY1ZxDJX8o&t=11s), [extended paper](https://arxiv.org/abs/2102.07405), [short paper](https://arxiv.org/abs/2107.10884),
...
*  Covariance matrices of variational Gaussian inference [opper2009variational]
...
Our NGD method 
* generalizes the exponential natural evolutionary strategy [glasmachers2010exponential] 
* recovers existing  Newton-like algorithms 
* yields new structured 2nd-order methods and adaptive-gradient methods with group-structural invariance [lin2021structured]
...
* enable lower iteration cost compared to the matrix exponential map suggested in [glasmachers2010exponential]
</div>
...
Note that `$\mathbf{g}_\mu$` and `$\mathbf{g}_{\Sigma}$` are Euclidean gradients of `$\eqref{1}$` computed via Stein's lemma [opper2009variational] [lin2019stein] :
`$$
\begin{aligned}
...
The second-order term shown in red is used for the positive-definite constraint [lin2020handling] known as a retraction in Riemannian gradient descent (RGD).  The higher-order term `$O(\beta^3)$` will be used for structured precision matrices in the next section. 
...
>	
>Note that (see [lin2021tractable]  for the detail)
...
\mathbf{B}_{t+1}  & \leftarrow   \mathbf{B}_t \mathbf{h} \Big( \beta \mathbf{C}_{\text{up}} \odot \kappa_{\text{up}}\big( 2 \mathbf{B}_t^{-1} \mathbf{g}_{\Sigma_t} \mathbf{B}_t^{-T} \big) \Big)
 \end{aligned}
$$` 
* obtains an update to solve `$\eqref{5}$` with group-structural invariance [lin2021structured]:
`$$
\begin{aligned}
