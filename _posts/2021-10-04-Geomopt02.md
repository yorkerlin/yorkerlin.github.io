...
In K-[dimensional]({{ site.baseurl }}{% post_url 2021-09-06-Geomopt01 %}#dimensionality-of-a-manifold) manifold cases, an intrinsic parameter space `$\Omega_\tau$` is often a **subspace** of `$\mathcal{R}^K$` while the gradient space  `$T_{\tau_0} (\Omega_\tau)$` is the whole space `$\mathcal{R}^K$` (see the following figure as an illustration).
