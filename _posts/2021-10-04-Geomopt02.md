...
Under parameterization $\tau$, we can locally re-expressed the function as `$h(\mathbf{\tau}):=h( (\tau_x,\tau_y,\sqrt{1-\tau_x^2-\tau_y^2}) )$` where `$\tau \in \Omega_\tau$`.
...
The parametric  representation of the vector is denoted by  `$\mathbf{v}(\mathbf{\tau_0})$`, where `$\mathbf{v}(\mathbf{\tau_0})^T \nabla_\tau h(\mathbf{\tau_0}):=\frac{d h(\gamma(t)) }{d t} \Big|_{t=0} $` for any smooth scalar function $h$ defined in the manifold.


This is the **geometric view** of a Riemannian gradient. The nice property of this definition is that a tangent vector is invariance to any **intrinsic** reparameterization.
