...
> Example 2: 
> given a prior $$ p(z) $$ and a likelihood `$ p(\mathcal{D} | z ) $` with observations $ \mathcal{D} $, we can approximate the exact posterior $ p( z | \mathcal{D} )  $ by solving a variational inference problem with 
...
> Example 3: 
> in gradient-free optimization, we often use a search distribution `$ \pi(a | \tau ) $` to find the optimal solution of `$f(a)$` by solving
>`$$
\begin{aligned}
\min_{\tau}  E_{ \color{red}  {\pi(a | \tau)} } [ f(a)  ]  
...
In these examples, we express the objective function in terms of an expectation highlighted in red.
