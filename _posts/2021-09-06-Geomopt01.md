...
Informally, a (smooth) manifold should be locally like a (flat) vector space. For example, consider the (curved) Earth surface looks like a (locally) flat space for us.  
...
Clearly, a point $ (0,1) $ highlighted in green is in the circle.
...
we show that the manifold does **not** look like a flat space under the following parametrization
...
------
We define a dimension of a manifold by using the dimension of an intrinsic parametrization. Formally, we can show that any intrinsic parametrization of a manifold has the same degrees of freedom. 
...
It is indeed a manifold with (closed) boundary. We will not consider this case in these blog posts.
