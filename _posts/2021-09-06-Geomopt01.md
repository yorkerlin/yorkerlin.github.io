---
title: 'Basics of Information Geometry (part 1)'
date: 2021-09-06
...
This kind of parametrization is **not** an intrinsic parameterization.

<img src="/img/2d-perturbation.png" title="Fig" width="300"/>

Parametrization 3:

The following parametrization of the circle is **not** considered to be an intrinsic parameterization.
...
This smoothness requirement of the inverse map is essential when it comes to reparametrization (A.K.A. parameter transformation) for stastical manifolds.
The  requirement gives us a way to generate new **intrinsic** parameterizations.
...
In this example, we will show that the regularity condition holds. Consider a 1-dimensional Gaussian family $ \\{ \mathcal{N}(w \|\mu,\sigma) \Big\| \mu \in \mathcal{R}, \sigma>0 \\}$ with parameter $\tau = (\mu,\sigma) $.
...
In this example, we will show that the regularity condition fails. Consider a Bernoulli family $ \\{ \mathcal{I}(w=0) \frac{\pi_0}{\pi_0+\pi_1} + \mathcal{I}(w=1) \frac{\pi_1}{\pi_0+\pi_1} \Big\| \pi_0>0, \pi_1>0  \\}$ with parameter $\tau = (\pi_0,\pi_1) $, where function $ \mathcal{I}(\cdot) $ is the indicator function.
The partial derivatives are
...
We will soon show that the  condition also fails for Bernoulli family  $ \\{ \mathcal{I}(w=0) \pi_0+ \mathcal{I}(w=1) \pi_1  \Big\| \pi_0>0, \pi_1>0, \pi_0+\pi_1=1  \\}$ with parameter $\tau = (\pi_0,\pi_1)$.  


A remark is that the condition does hold for Bernoulli family $ \\{ \mathcal{I}(w=0) \pi_0+ \mathcal{I}(w=1) (1-\pi_0)  \Big\| 0<\pi_0<1  \\}$ with parameter $\tau = \pi_0$.
...
It is possible to define the Fisher matrix under a non-intrinstic parameterization. However, the Fisher matrix can be singular or ill-defined under this parameterization.
...
Note that the Fisher matrix can be ill-defined under a **non-intrinsic** parameterization.
...
Consider another Bernoulli family $ \\{ \mathcal{I}(w=0) \frac{\pi_0}{\pi_0+\pi_1} + \mathcal{I}(w=1) \frac{\pi_1}{\pi_0+\pi_1} \Big\| \pi_0>0, \pi_1>0  \\}$ with parameter $\tau = (\pi_0,\pi_1) $.
